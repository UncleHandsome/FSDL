...
5. 🚨【理氣全周天連續性與空亡線（Circular Continuous Li Qi）】:
   ...
   - 出卦與空亡線判定：令 d_edge = |(deg % 15 + 15) % 15 - 7.5|，此為「距最近山脈分界線之角距離」（山心 d_edge = 7.5 最穩、騎線 d_edge = 0 空亡最凶），凶度隨 d_edge 連續衰減，呈現兼向分級效果（漸離漸穩）。嚴禁把公式寫成到山心的距離（相位反轉會把吉凶整個顛倒）！
...
⚡⚡⚡【全域物理湧現、去硬編碼與代碼修改十二全天條（每輪強制重申・違者退回）】⚡⚡⚡
